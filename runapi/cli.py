...
@app.command()
def generate(
    item: str = typer.Argument(..., help="What to generate (route, schema, repository, service, main, middleware)"),
...
'''
    ...
    console.print(f"📦 Classes: {class_name}Base, {class_name}Create, {class_name}Update, {class_name}Response, {class_name}List")
...
def schemas():
    ...
    for schema_file in schemas_path.rglob("*.py"):
        ...
        module_path = "schemas." + str(relative_path).replace("\\", ".").replace("/", ".").replace(".py", "")
